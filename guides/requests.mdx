...
```bash
  curl --request POST \
    --url http://api.pipevest.com/v1/requests/purchase\
    ...
    ...
    --data '{"customerId": 123456, "currency": "ZK", "instrumentId": 123456, "purchaseAmount": 100 }'
```
...
```bash
  curl --request POST \
    --url http://api.pipevest.com/v1/requests/purchase/sell\
    ...
    --data '{"customerId": 123456, "currency": "ZK", "assetId": 123456, "sellAmount": 100 }'
```
...
```bash
  curl --request POST \
    --url http://api.pipevest.com/v1/requests/purchase/purchase\
    ...
...
```bash
  curl --request PUT \
    --url http://api.pipevest.com/v1/requests\
    ...
...
```bash
  curl --request GET \
    --url http://api.pipevest.com/v1/requests?customerId=123456\
    --header 'Authorization: Bearer 123456' \
    --header 'Signature: sig1=:OTEyMjY4...A5NTNDMEQ=:' \
    --header 'Signature-Input: sig1=("Authorization" "Content-Digest" "Content-Length" "Content-Type" "X-Client-Id" "X-Idempotency-Key" "@method" "@target-uri" "@path" "@query");keyid="staging-pipevest-ed25519";created=1732893484;expires=1732893584' \
    --header 'X-Client-Id: 123456' \
...
```bash
  curl --request GET \
    --url http://api.pipevest.com/v1/requests/123456\
    ...
```
